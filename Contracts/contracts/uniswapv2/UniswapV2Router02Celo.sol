...
contract UniswapV2Router02Celo is IUniswapV2Router02 {
        ...
        returns (uint256 amountToken, uint256 amountETH, uint256 liquidity)
    {
        ...
        console.log('ROUTER 3.0 Token balance: %d',  IERC20Uniswap(token).balanceOf(pair) );
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        console.log('ROUTER 3.0 Token balance fafter: %d',  IERC20Uniswap(token).balanceOf(pair) );

        console.log('ROUTER 2. amountETH %d, amountETHMin %d', amountETH, amountETHMin);

        console.log('ROUTER 3.0 WETH BAL: %d , Pair balance: %d', IERC20Uniswap(WETH).balanceOf(address(this)), IERC20Uniswap(WETH).balanceOf(pair) );
        console.log('ROUTER 3.1 . P BLLA or %d', pair.balance );
        // TransferHelper.safeTransfer(WETH, pair, amountETH);
        TransferHelper.safeTransferETH( pair, amountETH);
        // bool b = IERC20Uniswap(WETH).transfer( pair, amountETH);
        //// require(b, 'WEtH TRANSFER FAILED');
        console.log('ROUTER 3.1 .  WETH BAL: %d, Pair balance %d ', IERC20Uniswap(WETH).balanceOf(address(this)), IERC20Uniswap(WETH).balanceOf(pair));
        console.log('ROUTER 3.1 . P BLLA or %d', pair.balance );
        liquidity = IUniswapV2Pair(pair).mint(to);
        console.log('ROUTER 4');
        // refund dust eth, if any
        if (msg.value > amountETH){
            console.log('ROUTER 5 1');
